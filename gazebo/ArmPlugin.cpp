...
void ArmPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) 
{
	...
	*/
	...
	/
	*/
	collisionSub = collisionNode(COLLISION_TOPIC, &ArmPlugin::onCollisionMsg, this);
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		*/
		...
		const bool collisionPropCheck = (strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0);
		
		if (collisionPropCheck)
		{
			const bool collisionGripperCheck = (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0);

			// issue reward after checking collision of prop and gripper
			rewardHistory = collisionGripperCheck ? REWARD_WIN : REWARD_LOSS;
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		const math::Box& propBoundingBox = prop->model->GetBoundingBox();
		...
		const math::Box& gripBoundingBox = gripper->GetBoundingBox();
		// z value for ground contact
		...
		*/
		...
		// check for ground contact from condition 
		const bool checkGroundContact = ( gripBoundingBox.min.z <= groundContact || gripBoundingBox.max.z <= groundContact );
		
		// issue a reward for robotic arm hitting the ground
		...
		*/
		...
		if(!checkGroundContact)
		{
			const float distGoal = BoxDistance(gripBoundingBox, propBoundingBox); // compute the reward from distance to the goal
			...
			if( episodeFrames > 1 )
			{
				...
				rewardHistory = INTERIM_REWARD * avgGoalDelta - INTERIM_OFFSSET;
